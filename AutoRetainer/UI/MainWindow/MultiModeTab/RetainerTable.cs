            ...
            foreach(var ret in retainerData)
            {
                ...
                }
                {
                    if(C.EntrustPlans.TryGetFirst(s => s.Guid == adata.EntrustPlan, out var plan))
                    {
                        ...
                        ImGui.PopFont();
                        ImGuiEx.Tooltip($"正在使用委託計畫「{plan.Name}」。" + (plan.ManualPlan ? "\n這是手動處理計畫。" : "") + (Utils.GetReachableRetainerBell(false) != null ? "\n點擊以執行委託。" : ""));
                ...
                ImGuiEx.Text($"{(!ret.HasVenture ? "沒有探險" : Utils.ToTimeString(ret.GetVentureSecondsRemaining(C.TimerAllowNegative)))}");
                ImGui.TableNextColumn();
                ImGui.TableSetBgColor(ImGuiTableBgTarget.CellBg, 0);
