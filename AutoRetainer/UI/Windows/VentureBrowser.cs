...
internal class VentureBrowser : Window
{
    ...
    public VentureBrowser() : base("委託瀏覽器")
    {
    ...
    public override void Draw()
    {
        ...
        if(ImGui.BeginCombo("##selectRet", SelectedCharacter != null ? $"{Censor.Character(SelectedCharacter.Name, SelectedCharacter.World)}－{Censor.Retainer(SelectedRetainer.Name)}－{SelectedRetainer.Level} {ExcelJobHelper.GetJobNameById(SelectedRetainer.Job)}" : "選擇雇員……", ImGuiComboFlags.HeightLarge))
        {
        ...
        if(SelectedRetainer != null && SelectedCharacter != null)
        {
            ...
            {
                ImGuiEx.TextCentered($"{Lang.CharLevel}{SelectedRetainer.Level} {ExcelJobHelper.GetJobNameById(SelectedRetainer.Job)}｜獲得力：{adata.Gathering}（{adata.Gathering / (float)MaxGathering:P0}）｜鑑別力：{adata.Perception}（{adata.Perception / (float)MaxPerception:P0}）");
            }
            else
            {
                ImGuiEx.TextCentered($"{Lang.CharLevel}{SelectedRetainer.Level} {ExcelJobHelper.GetJobNameById(SelectedRetainer.Job)}｜平均物品品級：{adata.Ilvl}（{adata.Ilvl / (float)MaxPerception:P0}）");
            }
            ...
            {
                ImGuiEx.TextWrapped($"缺少此雇員的資料。請使用傳喚鈴並選擇該雇員，以取得資料。");
            }
            else
            {
                if(Data.Count == 0)
                ...
                if(ImGui.BeginTable("##VentureBrowserTable", 9, ImGuiTableFlags.SizingFixedFit | ImGuiTableFlags.Borders | ImGuiTableFlags.RowBg | ImGuiTableFlags.ScrollY))
                {
                    ...
                    foreach(var x in Data.Where(x => x.VentureName.Contains(search, StringComparison.OrdinalIgnoreCase) && x.VentureLevel >= minLevel && x.VentureLevel <= maxLevel))
                    {
                        ...
                        if(ImGui.SmallButton($"加入規劃器##{x.ID}"))
                        {
                            adata.VenturePlan.List.Add(new(x.ID));
                        }
                        ImGui.SameLine();
                        if(ImGui.SmallButton($"查詢價格##{x.ID}"))
                        {
